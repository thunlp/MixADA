...
class PSOAttacker(Attacker):
    ...
    def generate_population(self, clsf,x_orig,neighbours_list,neighbours_len,x_len, target, targeted):
        ...
        ## for speed-up, we simplify the initialization process here.
        # h_score = np.array(self.norm(neighbours_len))
        # # w_list = [n_list[0] for n_list in neighbours_list]
        # w_list = []
        # for i in range(x_len):
        #     if neighbours_len[i] == 0:
        #         w_list.append(x_orig[i])
        #     else:
        #         w_list.append(neighbours_list[i][0])
    ...
    def get_neighbour_num(self, word, pos):
        try:
            return min(len(self.config["substitute"](word, pos)), 20) ## max: 20 substitutes
